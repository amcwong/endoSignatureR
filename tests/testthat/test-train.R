...
test_that("calibration improves Brier Score", {
    skip_if_not_installed("glmnet")
    skip_if_not_installed("rsample")
    skip_if_not(exists("gse201926_trainmini", where = asNamespace("endoSignatureR"), mode = "any"))
    ...
    expect_true("ece" %in% names(result_platt$metrics))
    expect_true("calibration" %in% names(result_platt))
    expect_true("method" %in% names(result_platt$calibration))

    # Verify calibrated probabilities exist
    expect_true("prob_calibrated" %in% names(result_platt$metrics$predictions))

    # Verify calibrated probabilities are in [0, 1]
    if (nrow(result_platt$metrics$predictions) > 0) {
        expect_true(all(result_platt$metrics$predictions$prob_calibrated >= 0))
        expect_true(all(result_platt$metrics$predictions$prob_calibrated <= 1))
    }
})

test_that("calibration metrics are valid", {
    skip_if_not_installed("glmnet")
    skip_if_not_installed("rsample")
    skip_if_not(exists("gse201926_trainmini", where = asNamespace("endoSignatureR"), mode = "any"))
    ...
    if (!is.na(result$metrics$brier_score)) {
        expect_true(result$metrics$brier_score >= 0)
        expect_true(result$metrics$brier_score <= 1)
    }

    # Verify ECE is non-negative or NA
    if (!is.na(result$metrics$ece)) {
        expect_true(result$metrics$ece >= 0)
    }
})

test_that("stability frequencies are valid", {
    skip_if_not_installed("glmnet")
    skip_if_not_installed("rsample")
    skip_if_not(exists("gse201926_trainmini", where = asNamespace("endoSignatureR"), mode = "any"))
    ...
    expect_true("bootstrap_frequency" %in% names(result$stability))
    expect_true("stable_genes" %in% names(result$stability))

    # Verify bootstrap frequencies are in [0, 1] if computed
    if (length(result$stability$bootstrap_frequency) > 0) {
        expect_true(all(result$stability$bootstrap_frequency >= 0))
        expect_true(all(result$stability$bootstrap_frequency <= 1))
    }
})

test_that("calibration works with isotonic regression", {
    skip_if_not_installed("glmnet")
    skip_if_not_installed("rsample")
    skip_if_not(exists("gse201926_trainmini", where = asNamespace("endoSignatureR"), mode = "any"))
    ...
    expect_equal(result$calibration$method, "isotonic")
    expect_true("prob_calibrated" %in% names(result$metrics$predictions))

    # Verify calibrated probabilities are in [0, 1]
    if (nrow(result$metrics$predictions) > 0) {
        expect_true(all(result$metrics$predictions$prob_calibrated >= 0))
