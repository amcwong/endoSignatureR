...
**Alternative**: You can also try `BiocManager::install()` which handles both CRAN and Bioconductor dependencies automatically:
...
The package provides three complementary workflows for endometrial RNA-seq analysis:

![Mode 1: Rapid Classification](inst/extdata/mode_1.png)

![Mode 2: Signature Validation](inst/extdata/mode_2.png)

![Mode 3: Visualization & Analysis](inst/extdata/mode_3.png)

Here are basic examples of how to use `EndoSignatureR` outside of the Shiny application:
...
**Mode 3: Visualization & Analysis** - Perform QC and exploratory analysis:
...
- **`esr_validateEndometrial(X, pheno, annot = NULL, label_col = "group")`**: Performs endometrial-specific schema/ID checks; returns cleaned objects and issues table.

- **`esr_loadPretrainedSignature()`**: Loads the shipped PS vs PIS signature from GSE201926.

- **`esr_classifyEndometrial(X_new, signature = NULL, threshold = 0.5, confidence = TRUE)`**: Applies pre-trained signature to new samples; returns predictions with confidence intervals.

- **`esr_trainEndometrialSignature(...)`**: Trains a new signature using in-fold transforms, DE screening, nested CV, LASSO, calibration, and optional stability selection.

- **`esr_compareSignatures(pretrained_result, new_result)`**: Compares pre-trained vs new signature performance.

- **`esr_analyzeDifferentialExpression(mat_t, pheno, label_col = "group")`**: Performs differential expression analysis using limma.

- **`esr_selectTopGenes(...)`**: Selects top genes by variance or differential expression statistics.

- **`esr_selectDEInFold(...)`**: Selects differentially expressed genes within cross-validation folds (anti-leakage).

- **`esr_transformInFold(...)`**: Transforms counts within cross-validation folds (anti-leakage).

- **`esr_transform_log1p_cpm(counts)`**: Transforms raw counts to log1p-CPM.

- **`esr_computeQCMetrics(counts, pheno)`**: Computes quality control metrics.

- **`esr_computeConfusionMatrix(predictions, truth, threshold = 0.5)`**: Computes confusion matrix from predictions.

- **`esr_compareThresholds(predictions, truth, thresholds = seq(0.1, 0.9, 0.1))`**: Compares classification performance across different thresholds.

- **`esr_createAnalysisBundle(...)`**: Creates a bundle of analysis results for export.

### Data Loading Functions

- **`endo_load_gse201926()`**: Loads GSE201926 dataset from extdata files.

- **`endo_load_demo()`**: Loads demo dataset for quick testing.

- **`endo_load_tpm(file_path)`**: Loads TPM counts from file.

- **`endo_load_annotation(file_path)`**: Loads gene annotation from file.

- **`endo_parse_metadata(series_matrix_file)`**: Parses GEO series matrix metadata.

- **`esr_loadFromFiles(counts_file, pheno_file, annot_file = NULL)`**: Loads data from multiple files.

- **`esr_loadCountsFromFile(file_path)`**: Loads counts matrix from file.

- **`esr_loadPhenoFromFile(file_path)`**: Loads phenotype data from file.

- **`esr_loadAnnotFromFile(file_path)`**: Loads annotation data from file.

### Plotting Functions

**QC/EDA:**

- **`plotEndometrialPCA(mat_t, pheno = NULL, ...)`**: Principal component analysis plot.
- **`plotEndometrialLibsize(counts, pheno = NULL, ...)`**: Library size distribution plot.
- **`plotEndometrialZeros(counts, by = "gene", ...)`**: Zero count percentage visualization.

**Differential Expression/Signature:**

- **`plotEndometrialMA(de_table, ...)`**: MA plot for differential expression results.
- **`plotEndometrialVolcano(de_table, ...)`**: Volcano plot for differential expression results.
- **`plotEndometrialHeatmap(mat_t, genes, pheno = NULL, ...)`**: Heatmap visualization.
- **`plotEndometrialCoefLollipop(signature, annot = NULL, ...)`**: Coefficient lollipop plot for signature genes.
- **`plotEndometrialStabilityBars(signature, ...)`**: Stability selection frequency bars.

**Performance:**

- **`plotEndometrialROC(predictions, use_calibrated = FALSE, ...)`**: ROC curve visualization.
- **`plotEndometrialPR(predictions, use_calibrated = FALSE, ...)`**: Precision-Recall curve visualization.
- **`plotEndometrialCalibration(predictions, use_calibrated = FALSE, ...)`**: Calibration curve visualization.
- **`plotEndometrialComparison(pretrained_result, new_result, ...)`**: Comparison plots between signatures.

### Export Functions

- **`esr_exportSignature(signature, result = NULL, dir = ".", formats = c("csv", "json"))`**: Exports signature to CSV/JSON formats.

- **`esr_exportPredictions(predictions, dir = ".", filename = "predictions.csv")`**: Exports predictions to CSV.

### Shiny Application

- **`runEndoSignatureR()`**: Launches the Shiny application for interactive data exploration.
...
The complete GSE201926 endometrial lesion RNA-seq dataset containing all genes across 12 samples (6 progestin-sensitive (PS) and 6 progestin-insensitive (PIS) patients). This is a list with three components:

- **`counts`**: Matrix of raw read counts (39,368 genes × 12 samples). Rows are genes (numeric GeneIDs), columns are samples (GSM IDs).
- **`pheno`**: Data frame with sample metadata containing `sample_id`, `group` (PS/PIS), and `title` columns.
- **`annot`**: Data frame with gene annotation containing `GeneID`, `Symbol`, `Description`, `GeneType`, `EnsemblGeneID`, and other annotation fields.

### `gse201926_trainmini`

A medium-sized training subset of the GSE201926 dataset containing 800-1500 genes across 12 samples, designed for fast nested cross-validation in vignettes and tests. Structure is the same as `gse201926_sample` but with fewer genes selected based on variance.

### `gse201926_annot_min`

A minimal gene annotation table for GSE201926 containing essential fields (`gene_id`, `symbol`, `biotype`) for mapping and display.

### `gse201926_pheno_min`

A minimal phenotype/metadata table for GSE201926 containing sample identifiers, group labels (PS/PIS), and optional batch information.

### `folds_demo`

Pre-computed cross-validation splits with fixed seeds for reproducible examples. Contains outer and inner CV splits for nested cross-validation workflows.

All datasets are derived from the GSE201926 study investigating progestin sensitivity in endometrial lesions. Source: <https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE201926>

## References

- Allaire, J. J., Xie, Y., Dervieux, C., McPherson, J., Luraschi, J., Ushey, K., Atkins, A., Wickham, H., Cheng, J., Chang, W., & Iannone, R. (2025). rmarkdown: Dynamic documents for R. R package. <https://CRAN.R-project.org/package=rmarkdown>.
...
- Ismail, E. A. R. (2015). Behavior of Lasso Quantile Regression with Small Sample Sizes. Journal of Multidisciplinary Engineering Science and Technology (JMEST), 2(3), March 2015. ISSN: 3159-0040. <https://www.jmest.org/wp-content/uploads/JMESTN42350527.pdf>.
...
- Wickham, H., Hester, J., Chang, W., & Bryan, J. (2025). devtools: Tools to Make Developing R Packages Easier. R package. <https://CRAN.R-project.org/package=devtools>.

- Wickham, H., Hester, J., & Bryan, J. (2025). readr: Read rectangular text data. R package. <https://CRAN.R-project.org/package=readr>.
