...
vignette: >
  %\VignetteIndexEntry{Mode 1: Rapid Classification Using Pre-trained Signature}
...
Mode 1 requires unlabeled counts data. Load your counts file using `esr_loadFromFiles()` or `esr_loadCountsFromFile()`:

```{r mode1-data-loading, eval=FALSE}
# Option 1: High-level convenience function (recommended)
...
X_new <- user_data$counts

# Option 2: Load counts directly
X_new <- esr_loadCountsFromFile(
  file_path = "my_counts.tsv",
...
# Or save to RDS format (recommended)
saveRDS(list(predictions = predictions, signature = signature),
        file = "classification_results.rds")
```
...
- Visualization: @ggplot22016 for base graphics and @complexheatmap2016 for heatmap visualizations
